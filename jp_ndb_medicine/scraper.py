    ...
    def _process_link_public_fund(self, tag, nth: int) -> None:
        ...
        for a in tag.find_all("a"):
            ...
            if "公費レセプトを含まないデータ" in label:
                ...
                self.fileinfo_list.append(FileInfo(url=link, nth=nth, public_fund=False, is_zip_file=True))

            elif "公費レセプトを含むデータ" in label:
                link = urljoin(DOMAIN_MHLW, a.attrs["href"])
                self.fileinfo_list.append(FileInfo(url=link, nth=nth, public_fund=True, is_zip_file=True))
